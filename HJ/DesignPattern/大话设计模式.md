...
| 对比项 | 工厂方法                   | 抽象工厂                            |
| ------ | -------------------------- | ----------------------------------- |
| 目的   | 创建一个产品族中的一个产品 | 创建多个相关或依赖产品族            |
| 粒度   | 单个产品                   | 产品族(多个产品)                  |
| 扩展性 | 易扩展新产品               | 易扩展新族,难扩展新产品类型        |
| 示例   | 不同数据库连接工厂         | GUI控件族(Win/Mac风格按钮+文本框) |

> 💡 记忆口诀: **“工厂方法造一个,抽象工厂造一套”**

### 2. 建造者 vs 抽象工厂

|          | 建造者                                     | 抽象工厂                   |
| -------- | ------------------------------------------ | -------------------------- |
| 关注点   | 分步骤构建复杂对象                         | 一次性创建产品族           |
| 产品结构 | 同一类对象,不同配置                       | 不同类型对象组合           |
| 典型场景 | 构建复杂对象(如StringBuilder、SQL构造器) | 跨平台UI组件、数据库产品族 |
...
| ---- | ----------------------- | ------------------------------ |
| 核心 | 克隆已有对象            | 全局唯一实例                   |
| 用途 | 避免重复构造开销        | 控制资源访问(如配置、连接池) |
| 实现 | 实现 Cloneable / copy() | 私有构造 + 静态 getInstance()  |
...
|      | 适配器                 | 桥接                           | 外观                         |
| ---- | ---------------------- | ------------------------------ | ---------------------------- |
| 目的 | 使不兼容接口能一起工作 | 分离抽象与实现,使二者独立变化 | 提供统一简化接口,封装子系统 |
| 类比 | 插座转换器             | 遥控器(抽象) + 电视(实现)  | 前台接待(统一入口)         |
| 关键 | 包装旧接口             | 解耦抽象和实现                 | 封装复杂性                   |

> 💡 记忆口诀: **“适配旧接口,桥接两独立,外观包复杂”**

### 2. 装饰器 vs 代理

|          | 装饰器                   | 代理                           |
| -------- | ------------------------ | ------------------------------ |
| 目的     | 动态添加职责(功能增强) | 控制访问(权限、延迟、日志等) |
| 结构     | 包装对象,接口相同       | 包装对象,接口相同             |
| 本质区别 | **增强功能**             | **控制访问**                   |
| 示例     | Java I/O 流包装          | 远程代理、缓存代理、权限代理   |
...
|      | 组合                    | 享元                         |
| ---- | ----------------------- | ---------------------------- |
| 目的 | 树形结构统一处理        | 共享对象,节省内存           |
| 重点 | 层次结构(部分-整体)   | 内部状态共享,外部状态分离   |
| 示例 | 文件系统(文件+文件夹) | 文字编辑器字符对象、游戏子弹 |
...
|        | 策略               | 状态                     | 模板方法                   |
| ------ | ------------------ | ------------------------ | -------------------------- |
| 核心   | 算法可替换         | 对象行为随状态改变       | 定义算法骨架,子类实现步骤 |
| 变化点 | 算法本身           | 状态                     | 步骤的具体实现             |
| 控制权 | 客户端选择策略     | 对象内部根据状态自动切换 | 父类控制流程               |
| 示例   | 支付方式、排序算法 | 订单状态机、游戏角色状态 | 框架生命周期、算法流程     |
...
> 注意: 发布-订阅是观察者模式的扩展(常通过消息队列解耦)

|           | 观察者                   | 发布-订阅(非GoF,但常提) |
| --------- | ------------------------ | -------------------------- |
| 耦合度    | 观察者直接注册到目标     | 通过中间消息队列解耦       |
| 同步/异步 | 通常同步                 | 通常异步                   |
| 示例      | GUI事件、Java Observable | 消息中间件、前端EventBus   |

> 💡 GoF只有“观察者”,发布订阅是其分布式/异步演进。

### 3. 命令 vs 责任链

|      | 命令                         | 责任链                   |
| ---- | ---------------------------- | ------------------------ |
| 目的 | 封装请求为对象               | 请求沿链传递,直到被处理 |
| 结构 | 请求封装成对象(含执行方法) | 多个处理器组成链         |
| 重点 | 支持撤销、队列、日志         | 动态决定谁处理           |
| 示例 | 菜单操作、事务               | 审批流程、异常处理链     |
...
|          | 中介者                            | 观察者                 |
| -------- | --------------------------------- | ---------------------- |
| 通信方式 | 对象通过中介者通信                | 目标直接通知观察者     |
| 耦合度   | 降低对象间耦合(星型结构)        | 目标与观察者仍直接耦合 |
| 示例     | 聊天室服务器、Air Traffic Control | 事件监听、数据绑定     |
...
|          | 备忘录                   | 命令                     |
| -------- | ------------------------ | ------------------------ |
| 撤销实现 | 保存对象状态快照         | 保存命令对象,执行undo() |
| 内存消耗 | 可能较大(保存完整状态) | 通常较小(只保存操作)   |
| 示例     | 游戏存档、文本编辑器历史 | 编辑器撤销、事务回滚     |
...
> **创建型(5)**: 工厂(方法+抽象)、单例、建造者、原型  
> → 记忆口诀: **“原单建抽方”**(原型、单例、建造、抽象工厂、工厂方法)

> **结构型(7)**: 适配、桥接、组合、装饰、外观、享元、代理  
> → 记忆口诀: **“适桥组,装外享代”**

> **行为型(11)**: 策模观,迭责命,备状访,中介解  
> → 记忆口诀: **“策模观迭责命,备状访中介解”**  
> (策略、模板、观察者、迭代器、责任链、命令、备忘录、状态、访问者、中介者、解释器)
...
| 行为型 | 策略、观察者、模板、状态、命令 | 业务逻辑、事件驱动、流程控制 |
...
1. 先掌握分类和每类解决什么问题。
2. 对比相似模式,理解其**本质区别**(目的、结构、控制权)。
3. 结合实际项目或框架(如 Spring、JDK、MyBatis)理解应用场景。
4. 画类图 + 写 Demo 加深理解。
...
掌握这些归类和对比,23种设计模式将不再难记！祝你学习顺利,设计模式融会贯通！ 🚀
